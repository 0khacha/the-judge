"""Trusted Sandbox & Execution Subprocess Isolation Protocol for The Judge v3.
...
Minimizes evaluation-specific information observable by untrusted code:
  - Anonymous, randomized temporary directories (no '_synthesized_tests' or 'judge' in paths)
  - Sanitized environment variables (stripping PYTEST_CURRENT_TEST, custom Judge markers)
  - Sanitized sys.argv (neutral test script filenames)
  - Independent fresh process boundaries for state-reset property verification
"""
...
class SandboxRunner:
    ...
    def sanitize_environment(extra_env: Optional[Dict[str, str]] = None) -> Dict[str, str]:
        ...
        # Standard system environment variables to preserve
        safe_keys = {
    ...
    def execute_in_anonymous_sandbox(
        cls,
        task_dir: str,
        test_scripts: List[Tuple[str, str]],  # List of (filename, code_content)
        pytest_args: Optional[List[str]] = None,
        timeout: int = 30
    ) -> Dict[str, Any]:
        """Execute test scripts in an anonymous, randomized temporary directory.
        
        Guarantees that filesystem paths, sys.argv, and directory names contain NO
        predictable Judge/synthesized test strings.
        """
        abs_task_dir = os.path.abspath(task_dir)
        
        # Create randomized anonymous temporary directory outside or inside task dir using generic hex names
        anon_dir_name = f"t_{uuid.uuid4().hex[:12]}"
        temp_dir = os.path.join(abs_task_dir, anon_dir_name)
        os.makedirs(temp_dir, exist_ok=True)

        try:
            # Create a conftest.py inside temp_dir to strip PYTEST_CURRENT_TEST right before function call execution
            ...
            # Execute pytest on neutral temporary directory
            ...
            # Post-execution challenge file integrity verification
            file_tampered = False
