...
def test_contract_parser_basic():
    ...
    spec_text = "The HTTP client must retry 5xx responses up to 3 times. Fast responses are optimal."
...
def test_hard_gate_9_coverage_gate():
    evidence = {
        ...
        "test_provenance": {
            "test_prop_1": {"source": "judge_challenge_test", "independence_level": "externally_verified"}
        },
        "challenge_manifest": {"file_tampered": False},
        "contract_data": {
            "summary": {
                "critical_unverified_ids": ["REQ-002"]
            }
        }
    }
